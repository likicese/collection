...
###### vim命令可通过加一个行号数字（用于指定某一行）或一对通过`,`或`;`分割的行号来指定文件范围
###### `a,b`是以当前行作为参考；`a;b`中b是以a行作为参考
###### `+` 或 `-` 表示当前行向下n行或当前行向上n行；如`:1,+3d`,`:1,-3d`,`:,5d`
...
| ---------------- | -------------------------------------------------------- |
| `i`          | 表示文本内部命令(inner)                   |
| `a`          | 表示文本周边命令(aroud)(包含一个空格)                   |
| `w`          | 表示单词(word)              |
| `s`          | 表示句子(sentence)          |
| `p`          | 表示段落(paragraph)          |
###### tips：一般说，`d`和命令`aw`、`as`、`ap`配合使用比较好，`c`和`iw`使用效果比较好。
...
| `b` or `B`      | 移动光标到上一个单词开头                |
| `e` or `E`      | 移动光标到所在单词末尾                |
| `ge` or `gE`     | 反向 移动到上一单词的结尾                |
| `*`      | 移动光标到所在单词的下一个单词                |
| `#`      | 移动光标到所在单词的上一个单词                |
###### 每个面向单词(word/以非字母分隔)的光标移动命令，都有一个面向字串(WORD/以空白字符分隔)的命令与其对应，分别为W、B、E 和 gE
...
| `尾行模式下`           | 说明                                                     |
| ---------------- | -------------------------------------------------------- |
|`:vsplit filename` or `:vsp filename`  | 垂直分屏（vertical）                |
|`:split filename` or `:sp filename`  | 水平分屏            |

| `命令模式下`           | 说明                                                     |
| ---------------- | -------------------------------------------------------- |
|`ctrl w` + `v`  | 垂直分屏（vertical）                |
|`ctrl w` + `s`  | 水平分屏            |
|`ctrl w` + `w`  | 在当前的分屏中顺时针切换            |
|`ctrl w` + `h` or `j` or `k` or `l`  | 上下左右切换分屏            |
|`ctrl w` + `c`  | 关闭当前分屏            |
...
##### 删除类命令
| `关于命令和对象`  | 许多改变文本的命令都是由一个操作符和一个动作构成                          |
| ----------------|-------------------------------------------------------- |
| `d` `motion`   | d为删除操作符                 |
| `motion`  | 操作符的操作对象                 |
...
| `使用计数指定动作`  | 在动作前输入数字会使它重复那么多次                          |
| ----------------|-------------------------------------------------------- |
| `0`    | 移动到光标行首               |
| `3w`    |                |
| `2e`    |                |
| `d` `number`(数字) `motion`    |      使用计数以删除更多          |
| `2dd`    |                |
...
| `c`  | change                          |
| ----------------|-------------------------------------------------------- |
| `c``e`   | 更改文本直到一个单词的末尾 `（ce不仅是删除了一个单词，同时也进入插入模式）`              |
| `c``w`   |               |
