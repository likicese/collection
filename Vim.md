...
| `Ctrl`+`r`&`Ctrl`+`a`     | 搜索模式下插入光标下的字串                |
| `Ctrl`+`o`    | 完成搜索后，光标跳转到开始搜索的位置               |
...
###### 3.3 `.`可用于重复执行命令； `/`搜索字符串，按`n`会跳转到下一个搜索到的目标字符串
例：`dw`删除单词，`5.`表示连续执行五次`dw`
    `/`查询到目标字符串，按`A`进入行尾追加模式，添加文本内容，`Esc`退出编辑模式，再执行`n.`对下一个目标字符串进行相同的添加
