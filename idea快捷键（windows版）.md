...
| `F7`            | 在 Debug 模式下，进入下一步，如果当前行断点是一个方法，则`进入当前方法体`内，如果该方法体还有方法，则不会进入该内嵌的方法中 |
| `F8`            | 在 Debug 模式下，进入下一步，如果当前行断点是一个方法，则`不进入当前方法体`内 |
...
| `Ctrl` + `Tab`       | 编辑窗口切换，加delete，则是关闭对应选中的窗口 |
| `Alt` + `↑`                                | 当前光标跳转到当前文件的前一个方法名位置 `（必备）`          |
| `Alt` + `↓`                                | 当前光标跳转到当前文件的后一个方法名位置 `（必备）`          |
